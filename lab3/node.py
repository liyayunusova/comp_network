...
from message import ChunkSharing
from message import Node2Node
from message import Node2Tracker
...
class Node:
    ...
    def send_chunk(self, filename: str, rng: tuple, dest_node_id: int, dest_port: int):
        ...
        log_content = "The process of sending a chunk to node{} of file {} has finished!".format(
            dest_node_id, filename)
        log(node_id=self.node_id, content=log_content)
    ...
    def listen(self):
        while True:
            data, addr = self.send_socket.recvfrom(
                config.constants.BUFFER_SIZE)
    ...
    def receive_chunk(self, filename: str, range: tuple, file_owner: tuple):
        ...
        self.send_segment(sock=temp_sock,
                          data=msg.encode(),
                          addr=tuple(dest_node["addr"]))
        log_content = "I sent a request for a chunk of {0} for node{1}".format(
            filename, dest_node["node_id"])
        log(node_id=self.node_id, content=log_content)

        while True:
            data, addr = temp_sock.recvfrom(config.constants.BUFFER_SIZE)
            # but this is not a simple message, it contains chunk's bytes
            msg = Message.decode(data)
    ...
    def split_file_owners(self, file_owners: list, filename: str):
        ...
        log(node_id=self.node_id, content=log_content)
        file_size = self.ask_file_size(
            filename=filename, file_owner=to_be_used_owners[0])
        ...
        chunks_ranges = [(round(step*i), round(step*(i+1)))
                         for i in range(len(to_be_used_owners))]
        ...
        for idx, obj in enumerate(to_be_used_owners):
            t = Thread(target=self.receive_chunk, args=(
                filename, chunks_ranges[idx], obj))
            t.setDaemon(True)
            t.start()
        ...
        log_content = "All the chunks of {} has downloaded from neighboring peers. But they must be reassembled!".format(
            filename)
        log(node_id=self.node_id, content=log_content)
    ...
    def inform_tracker_periodically(self, interval: int):
        ...
        Timer(next_call - time.time(), self.inform_tracker_periodically,
              args=(interval,)).start()
...
def run(args):
    ...
    timer_thread = Thread(target=node.inform_tracker_periodically, args=(
        config.constants.NODE_TIME_INTERVAL,))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-node_id', type=int,
                        help='id of the node you want to create')
