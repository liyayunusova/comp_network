...
from message import Tracker2Node
...
class Tracker:
    ...
    def add_file_owner(self, msg: dict, addr: tuple):
        ...
        self.file_owners_list[msg['filename']] = list(
            set(self.file_owners_list[msg['filename']]))
    ...
    def search_file(self, msg: dict, addr: tuple):
        ...
        for json_entry in self.file_owners_list[msg['filename']]:
            ...
            matched_entries.append(
                (entry, self.send_freq_list[entry['node_id']]))
    ...
    def check_nodes_periodically(self, interval: int):
        ...
        Timer(next_call - time.time(), self.check_nodes_periodically,
              args=(interval,)).start()
    ...
    def listen(self):
        timer_thread = Thread(target=self.check_nodes_periodically, args=(
            config.constants.TRACKER_TIME_INTERVAL,))
        timer_thread.setDaemon(True)
        timer_thread.start()

        while True:
            data, addr = self.tracker_socket.recvfrom(
